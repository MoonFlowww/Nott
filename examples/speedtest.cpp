...
struct NetImpl : torch::nn::Module {
    ...
    NetImpl() {
        ...
        pool1 = register_module("pool1", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions({2, 2}).stride({2, 2})));
        ...
        pool2 = register_module("pool2", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions({2, 2}).stride({2, 2})));

        conv5 = register_module("conv5", torch::nn::Conv2d(torch::nn::Conv2dOptions(64, 128, 3).stride(1).padding(1)));
        pool3 = register_module("pool3", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions({2, 2}).stride({2, 2})));
...
namespace LatencyUtils {
    ...
    StepLatencyStats build_stats_from_samples(const std::string &title, const std::vector<double> &samples,
                                              std::size_t warmup_steps = 100, double tukey_k = 1.0) {
...
inline Nott::Loss::Details::CrossEntropyDescriptor set(Nott::Model &model, const bool &device) {
    // Used to reset learnable parameters between Nott PreBuild and Nott Custom
    model.add(Nott::Layer::Conv2d(
        {.in_channels = 1, .out_channels = 32, .kernel_size = {3, 3}, .stride = {1, 1}, .padding = {1, 1}},
        Nott::Activation::ReLU, Nott::Initialization::HeUniform));
    model.add(Nott::Layer::Conv2d(
        {.in_channels = 32, .out_channels = 32, .kernel_size = {3, 3}, .stride = {1, 1}, .padding = {1, 1}},
        Nott::Activation::ReLU, Nott::Initialization::HeUniform));
...
static torch::Tensor stage_for_device(torch::Tensor tensor, const bool &device) {
    const auto dev = device ? torch::kCUDA : torch::kCPU;
...
static ClassificationMetrics evaluate_classifier(ForwardFn &&forward_fn, const torch::Tensor &data,
                                                 const torch::Tensor &labels, int64_t batch_size, bool use_cuda,
                                                 int64_t num_classes) {
...
int main() {
    ...
    model1.train(x1, y1, {
                     .epoch = epochs, .batch_size = B, .shuffle = false, .monitor = false, .buffer_vram = 0,
                     .graph_mode = Nott::GraphMode::Capture, .drop_last = true, .enable_amp = false,
    ...
    model1.eval();
    Nott_prebuilt_metrics = evaluate_classifier([&](const torch::Tensor &inputs) { return model1.forward(inputs); }, x2,
                                                y2, B, IsCuda, num_classes);


    // 50% Nott (manual training loop using Nott model
    ...
    for (int64_t e = 0; e < epochs; ++e) {
        for (int64_t i = 0; i < N; i += B) {
            ...
            double step_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
    ...
    Nott_custom_metrics = evaluate_classifier(
        [&](const torch::Tensor &inputs) { return model2.forward(inputs); },
        x2,
        y2,
        B,
        IsCuda,
    ...
    net->eval();
    libtorch_metrics = evaluate_classifier([&](const torch::Tensor &inputs) { return net->forward(inputs); }, x2, y2, B,
                                           IsCuda, num_classes);
    ...
    const LatencyUtils::StepLatencyStats Nott_prebuilt_stats =
            LatencyUtils::build_stats_from_samples("Nott Train() (Tukey 0.98)", Nott_prebuilt_samples, 1000, 0.98);

    const LatencyUtils::StepLatencyStats Nott_custom_stats =
            LatencyUtils::build_stats_from_samples("Nott + Custom Train() (Tukey 0.98)", Nott_custom_samples, 1000, 0.98);

    const LatencyUtils::StepLatencyStats libtorch_stats =
            LatencyUtils::build_stats_from_samples("Libtorch Raw (Tukey 0.98)", libtorch_samples, 1000, 0.98);
    ...
    std::cout << "\n\n Total Runtime: " << std::chrono::duration_cast<std::chrono::seconds>(
        std::chrono::high_resolution_clock::now() - t1).count() / 60.0f << "min" << std::endl;
    return 0;
}
