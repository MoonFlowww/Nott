...
namespace {
    ...
    torch::Tensor ConvertRgbMasksToOneHot(const torch::Tensor &masks) {
        ...
        auto palette = torch::tensor(palette_vec, torch::TensorOptions().dtype(torch::kUInt8)).view({C, 3}).to(
            uint8_masks.device());
    ...
    void ComputeDensityAndSpeed(const cv::Mat &image, const SuperpixelParams &params, cv::Mat &density,
                                cv::Mat &base_speed, double &total_density) {
        cv::Mat img32f;
        if (image.channels() == 3) {
    ...
    std::vector<cv::Point2f> InitializeCenters(const cv::Mat &image, int desired_superpixels, std::mt19937 &rng) {
        ...
        std::uniform_real_distribution<float> jitter(-0.25f * static_cast<float>(step),
                                                     0.25f * static_cast<float>(step));
    ...
    cv::Mat FastMarching(
        ...
        while (!queue.empty()) {
            ...
            if (current.cost > dist.at<float>(current.y, current.x) || finalized.at<
                    std::uint8_t>(current.y, current.x)) {
                continue;
            }
    ...
    std::vector<cv::Mat> ComputeDistanceMaps(
        ...
        for (std::size_t idx = 0; idx < centers.size(); ++idx) {
            cv::Point center_i(static_cast<int>(std::round(centers[idx].x)),
                               static_cast<int>(std::round(centers[idx].y)));
    ...
    void AssignLabels(const std::vector<cv::Mat> &distance_maps, cv::Mat &labels, cv::Mat &min_distance) {
    ...
    double ComputeEnergy(const cv::Mat &labels, const std::vector<cv::Mat> &distance_maps,
                         const SuperpixelParams &params, const std::vector<double> &areas, double avg_area) {
    ...
    void RelocateCenters(std::vector<cv::Point2f> &centers, const cv::Mat &labels,
                         const std::vector<cv::Mat> &distance_maps, const SuperpixelParams &params) {
        const int rows = labels.rows;
        const int cols = labels.cols;
        ...
        for (int l = 0; l < superpixel_count; ++l) {
            if (weights[static_cast<std::size_t>(l)] > 1e-6) {
                centers[static_cast<std::size_t>(l)].x =
                        new_centers[static_cast<std::size_t>(l)].x / static_cast<float>(weights[static_cast<std::size_t>
                            (l)]);
                centers[static_cast<std::size_t>(l)].y =
                        new_centers[static_cast<std::size_t>(l)].y / static_cast<float>(weights[static_cast<std::size_t>
                            (l)]);
            }
        }
    }
    ...
    void AttemptSplits(
        ...
        for (int i = 0; i < superpixel_count; ++i) {
            ...
            if (C_shape <= params.Tc && C_size <= params.Ts) {
                regions.push_back(RegionInfo{
                    i, areas[static_cast<std::size_t>(i)], c,
        ...
        if (!split_happened && static_cast<int>(centers.size()) < params.desired_superpixels) {
            ...
            for (int k = 0; k < max_additional && k < static_cast<int>(regions.size()); ++k) {
                ...
                if (static_cast<int>(centers.size() + new_centers.size()) >= params.desired_superpixels + params.
                    max_splits) {
                    break;
                }
            }
    ...
    inline std::vector<SuperpixelSegmentationResult> RunSuperpixelsOnBatch(
        const torch::Tensor &images, const SuperpixelParams &params) {
...
int _main() {
    ...
    for (int tile = 1; tile <= 1; ++tile) {
        // 9
        std::string path = "/home/moonfloww/Projects/DATASETS/Image/Satellite/DubaiSegmentationImages/Tile " +
                           std::to_string(tile);
        ...
        std::cout << "Estimator {X,Y}: " << EstX << " || n: " << EstX[0] * EstX[1] << " + " << (EstX[0] - 1) * (
            EstX[1] - 1) << std::endl;
    ...
    SuperpixelDataset ds = BuildSuperpixelDatasetForLimitedVoronoiCuts(X, Y, params, 20);
    //BuildSuperpixelDataset(X, Y, params);
    X = ds.X;
    Y = ds.y;
